...
public class BoardDAO {
	...
	public int deleteBoard(BoardDTO bDTO)throws SQLException{
		...
		//3.Connection얻기
			con=db.getDbConn();
		//4.쿼리문 생성객체 얻기
			StringBuilder deleteBoared=new StringBuilder();
			deleteBoared
			.append("	delete from board  ")
			.append("	where num=? and id=?	");
			
			pstmt=con.prepareStatement(deleteBoared.toString());
		//5.바인드변수에 값 할당
	...
	public int updateBoard(BoardDTO bDTO)throws SQLException{
		...
		try {
			...
			con=db.getDbConn();
			//4.쿼리문 생성객체 얻기
			StringBuilder updateBoared=new StringBuilder();
			updateBoared
			.append("	update board  ")
			.append("	set content=?  ")
			.append("	where num=? and id=?	");
			
			pstmt=con.prepareStatement(updateBoared.toString());
			//5.바인드변수에 값 할당
